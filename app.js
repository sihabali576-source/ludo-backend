...
function selectPaymentMethod(method, btn) {
    ...
    if(btn) {
        // Find all cards matching method and select them (to keep UI synced if tabs switch)
...
async function requestDeposit() {
  ...
  try{
    ...
    const res = await fetch("https://ludo-9j3s.onrender.com/deposit",{
      method:"POST",
...
async function requestWithdraw(){
  ...
  try{
    ...
    const res = await fetch("https://ludo-9j3s.onrender.com/withdraw",{
      method:"POST",
      headers:{ "Content-Type":"application/json" },
      body: JSON.stringify({
        amount: amount,
...
function renderFinance() {
    ...
    appState.transactions.forEach(t => {
        ...
        tr.innerHTML = `
            ...
            <td>${user}</td>
            <td><button class="btn-sm btn-approve" onclick="processTrx(${t.id}, 'success')">
<i class="fas fa-check"></i></button>

<button class="btn-sm btn-reject" onclick="processTrx(${t.id}, 'rejected')">
