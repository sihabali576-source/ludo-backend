...
// Login Logic (Modified to handle Admin Panel context if visible)
function handleLogin(event) {
    ...
    if (loginScreen && loginScreen.style.display !== 'none' && adminPassInput) {
        // Admin Panel Login Logic
        const pass = adminPassInput.value;
        if(pass === '1234') { // Secure PIN
...
let isGameResultProcessed = false; // Prevent double payout
...
function startGame(mode, count) {
    ...
    isGameResultProcessed = false; // Reset flag
...
function checkWinCondition(color) {
    if (boardState[color].filter(t => t.status === 'finished').length === 4) {
        ...
        if (gameMode === 'vsAI') {
            ...
            // AUTO RETURN TO DASHBOARD AFTER 2.5 SECONDS
            setTimeout(() => {
                finishGameSession();
            }, 2500);
        } else {
...
function saveState() {
    ...
    updateUI();
    renderHistory();
    renderAdminRequests(); // Update Admin if open
}
...
function finishGameSession() {
    // Guard clause: If already processed (Auto return), just navigate
    if (isGameResultProcessed) {
        isGameResultProcessed = false; // Reset for next game
        document.getElementById('win-modal').style.display = 'none';
        goToWalletDashboard();
        return;
    }
...
// ==========================================
// PART 3: ADMIN PANEL INTEGRATION (CONNECTOR)
// ==========================================
// This part connects the Full Screen Admin Panel HTML to appState
...
function switchView(view) {
    ...
    // Note: event.currentTarget might not be available if called programmatically, 
    // so we rely on the click handler in HTML usually, but here we just ensure UI updates.
...
// --- Admin Data Rendering (Connected to appState) ---

function renderFinance() {
    // Deposits
    const depTable = document.getElementById('deposit-table');
    depTable.innerHTML = `<thead><tr><th>Type</th><th>Method</th><th>Amount</th><th>User</th><th>Action</th></tr></thead><tbody></tbody>`;
    const depBody = depTable.querySelector('tbody');
    
    // Withdraws
    const withTable = document.getElementById('withdraw-table');
    withTable.innerHTML = `<thead><tr><th>Type</th><th>Method</th><th>Amount</th><th>User</th><th>Action</th></tr></thead><tbody></tbody>`;
    const withBody = withTable.querySelector('tbody');
    ...
    appState.transactions.forEach(t => {
        ...
        // Extract user name if possible, or just "Player"
        const user = t.info.split(' ')[0] || "Player"; 
        ...
        if(t.type === 'deposit') depBody.appendChild(tr);
        else {
    ...
    const pendCount = document.getElementById('fin-pending-with');
    if(pendCount) pendCount.innerText = pendingWith;
}

// Unified Transaction Processor for Admin Panel
function processTx(id, status) {
    const tx = appState.transactions.find(t => t.id === id);
    if(tx) {
        if (status === 'success') {
            if(tx.type === 'deposit') {
                appState.balance += tx.amount; // Logic: Admin adds to system balance? Or user balance?
                // In the simple appState model, there is no "per-user balance" in the admin panel view context,
                // BUT in the real app, this transaction needs to affect the specific user.
                // Since we are using a single `appState` for the local user in this demo, 
                // approving a deposit adds to the current user's balance (simulated).
                // NOTE: In a real backend, this would target specific user ID.
                // Here we assume the transactions in appState belong to the logged in user or are global.
                // For the purpose of this specific file (Client Side Only), we update appState.
            }
        } else {
            if(tx.type === 'withdraw') {
        ...
        tx.status = status;
        localStorage.setItem('ludoAppState', JSON.stringify(appState));
        renderFinance(); // Re-render admin tables
        alert('Transaction ' + status);
    }
}

function renderSettings() {
    const bkashInput = document.getElementById('set-bkash');
    const nagadInput = document.getElementById('set-nagad');
    const rocketInput = document.getElementById('set-rocket');

    if(bkashInput) bkashInput.value = appState.adminNumbers.bkash;
    if(nagadInput) nagadInput.value = appState.adminNumbers.nagash || appState.adminNumbers.nagad; // Typo safety
    if(nagadInput) nagadInput.value = appState.adminNumbers.nagad;
    if(rocketInput) rocketInput.value = appState.adminNumbers.rocket;
}
...
// --- Dummy Stats Generators (To keep Admin UI alive) ---
function updateStats() {
    // Live Rooms (Simulated)
    const activeRooms = document.getElementById('dash-active-rooms');
    if(activeRooms) activeRooms.innerText = Math.floor(Math.random() * 20) + 50;
}

function startDiceLogs() {
    ...
    setInterval(() => {
        ...
        line.innerHTML = `<span class="log-time">${new Date().toLocaleTimeString()}</span> Server RNG: Rolled <span class="log-success">${dice}</span> [Hash Validated]`;
        logDiv.prepend(line);
...
window.addEventListener('load', () => {
   saveState();
   // Check if Admin Panel needs to be shown (Persistent session)
